...
interface Props {
  ...
  totalDue: number;
  balance: number;
  orderStatus: string; // bottleneck, read-only here
}
...
export function ExpandableOrderRow({ receiptId, receiptNumber, custName, date, totalDue, balance, orderStatus }: Props) {
  ...
  return (
    <>
      <tr className="hover:bg-stone-25 dark:hover:bg-white/5 transition-colors">
        ...
        <td className="td"><OrderStatusBadge status={orderStatus} /></td>
      </tr>
      {expanded && (
        <tr>
          <td colSpan={7} className="td bg-stone-25 dark:bg-white/[0.02] py-3">
            {loading && (
                ...
                {items.map((item) => (
                  <li key={item.id} className="flex items-center justify-between gap-3">
                    ...
                    <OrderStatusSelect
                      ...
                      onChanged={(next) => updateCachedStatus(item.id, next)}
                    />
                  </li>
                ))}
              </ul>
